...
void unzip() {
	...
	ZIPENTRY ze;
	int numitems;
	UnzipOpenFileW(&huz, L"C:\\Users\\user\\Desktop\\ExamFile.zip", "1234"); //2번 : 압축파일 경로,3번 : 비밀번호
	UnzipSetBaseDirW(huz, L"C:\\Users\\user\\Desktop\\ExamFile"); //압축 해제할 장소
	ze.Index = -1;
	UnzipGetItem(huz, &ze);
	numitems = ze.Index;
...
void zip() {
	bool CreateZip;
	CreateZip = CZipper::ZipFolder("C:\\Users\\user\\Desktop\\ExamFile", FALSE); //해당 폴더를 압축시킴 - ziptest폴더도 같이 압축됨, 압축 후 상위(work)폴더에 압축파일 생성
}
int checksha(SOCKET sock) {
	...
	//파일 읽기
	fopen_s(&fp, "C:\\Users\\user\\Desktop\\ExamFile.zip", "rb");
	if (fp == NULL)
	{
...
void StartExam(HWND hWnd) {
	...
	Examstart = 1;	
	unzip();
	r = remove("C:\\Users\\user\\Desktop\\ExamFile.zip"); //받은 압축 파일 삭제
...
int SendFile(HWND hWnd) { // 서버로 파일 전송
	zip();
	FILE* fp = fopen("C:\\Users\\user\\Desktop\\ExamFile.zip", "rb"); // 현재 보낼 파일이 있는 위치 및 파일 이름
	int r = 0;
	...
	r = remove("C:\\Users\\user\\Desktop\\ExamFile.zip");//전송완료한 파일 삭제
	//r = remove("C:\\Users\\user\\Desktop\\ExamFile");//전송완료한 파일 삭제
	return 0;
}
...
void compulsionQuit(HWND hWnd, wchar_t* hak_buffer, wchar_t* name_buffer) {
	...
	if (1 == SendFile(NULL)) { // 파일전송에 실패하면
		...
		closesocket(sock); // 서버 소켓 종료
		WSACleanup(); // 소켓 종료
		int r = remove("C:\\Users\\user\\Desktop\\ExamFile.zip"); //받은 압축 파일 삭제
...
void recvFile(SOCKET sock) {
	...
	wchar_t buffer[BUFFERSIZE] = L"C:\\Users\\user\\Desktop\\";
	// 저장할 파일명 변수
...
LRESULT CALLBACK WndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_CLOSE:
		if (true == g_controlManager->getProcessTerminateMode())
		{
